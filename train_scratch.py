"""
Oil Slick Detection — ResNet-18 Trained from Scratch (no pretrained weights)

Identical pipeline to train.py with one change: pretrained_init=False.
The first conv layer is randomly initialised (no ImageNet weight blending).
The learning rate is raised from 5e-5 to 1e-3 because a randomly initialised
network needs larger steps to escape the random weight regime; the fine-tuning
LR used in train.py would converge too slowly from scratch.

Run: conda activate oilslick && python train_scratch.py
"""
...
RUN_ID    = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "_scratch"
...
# ── Exploratory plots (identical to train.py) ─────────────────────────────────
...
def plot_sample_chips(meta):
    n_show = 3
    pos_ids = meta[meta["label"] == 1]["sample_id"].sample(
        min(n_show, (meta["label"] == 1).sum()), random_state=41).tolist()
    neg_ids = meta[meta["label"] == 0]["sample_id"].sample(
        min(n_show, (meta["label"] == 0).sum()), random_state=41).tolist()
    for sample_ids, title, fname in [
        (pos_ids, "Positive Samples (Oil Slick Present)", "chips_positive.png"),
        (neg_ids, "Negative Samples (No Oil Slick)",      "chips_negative.png"),
    ]:
        n = len(sample_ids)
...
def plot_channel_stats(meta):
    ...
    all_vv, all_vh = [], []
    for sid in subset:
        chip = load_chip(sid); valid = chip != 0
        all_vv.append(chip[0][valid[0]]); all_vh.append(chip[1][valid[1]])
    all_vv = np.concatenate(all_vv); all_vh = np.concatenate(all_vh)
    print(f"VV — mean: {all_vv.mean():.4f}  std: {all_vv.std():.4f}")
    print(f"VH — mean: {all_vh.mean():.4f}  std: {all_vh.std():.4f}")
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    axes[0].hist(all_vv, bins=100, color="steelblue", alpha=0.7); axes[0].set_title("VV Distribution")
    axes[1].hist(all_vh, bins=100, color="darkorange", alpha=0.7); axes[1].set_title("VH Distribution")
...
class OilSlickDataset(Dataset):
    def __init__(self, split_file, img_dir, label_map,
        ...
        self.sample_ids = [sid for sid in all_ids if sid in available and sid in label_map]
        self.img_dir = img_dir; self.label_map = label_map
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            if torch.rand(1).item() > 0.5: chip = torch.flip(chip, dims=[2])
            if torch.rand(1).item() > 0.5: chip = torch.flip(chip, dims=[1])
            chip = torch.rot90(chip, torch.randint(0, 4, (1,)).item(), dims=[1, 2])
            chip = chip + self.noise_std * torch.randn_like(chip)
        return chip, torch.tensor(self.label_map[sid], dtype=torch.float32)


# ── Channel statistics (Welford) ──────────────────────────────────────────────
def compute_channel_stats(split_file, img_dir):
    available = set(f.replace("_s1.tif", "") for f in os.listdir(img_dir) if f.endswith(".tif"))
    with open(split_file) as f:
        train_ids = [sid for sid in (l.strip() for l in f) if sid in available]
    ...
    for i, sid in enumerate(train_ids):
        with rasterio.open(os.path.join(img_dir, f"{sid}_s1.tif")) as src:
            chip = src.read().astype(np.float32)
        chip[chip == -163.0] = np.nan
        chip[0] = np.clip(chip[0], -50, 10); chip[1] = np.clip(chip[1], -50, 10)
        for pixels, count, mean, M2 in [
            (chip[0][~np.isnan(chip[0])].ravel(), vv_count, vv_mean, vv_M2),
        ]:
            n = len(pixels)
            if n > 0:
                bm = pixels.mean(); d = bm - vv_mean
                vv_mean = (vv_mean * vv_count + bm * n) / (vv_count + n)
                vv_M2  += pixels.var() * n + d**2 * vv_count * n / (vv_count + n)
                vv_count += n
        vh_pixels = chip[1][~np.isnan(chip[1])].ravel()
        n = len(vh_pixels)
        if n > 0:
            bm = vh_pixels.mean(); d = bm - vh_mean
            vh_mean = (vh_mean * vh_count + bm * n) / (vh_count + n)
            vh_M2  += vh_pixels.var() * n + d**2 * vh_count * n / (vh_count + n)
            vh_count += n
        if (i + 1) % 100 == 0:
...
# ── Model — randomly initialised conv1, no pretrained weights ─────────────────
def _adapt_conv1_random(net):
    old = net.conv1
    new = nn.Conv2d(2, old.out_channels, kernel_size=old.kernel_size,
                    stride=old.stride, padding=old.padding, bias=False)
    # Xavier uniform init — matches PyTorch default for Conv2d
    nn.init.kaiming_uniform_(new.weight, mode="fan_out", nonlinearity="relu")
    net.conv1 = new


def build_resnet18_scratch(dropout=0.4):
    net = models.resnet18(weights=None)   # no pretrained weights
    _adapt_conv1_random(net)
...
def evaluate(model, loader, device):
    ...
    probs = torch.sigmoid(all_logits).numpy(); labels_np = all_labels.numpy().astype(int)
...
def train_model(model, train_loader, val_loader, device,
                epochs=80, lr=1e-3, weight_decay=5e-4, patience=20,
                label_smoothing=0.05, monitor="f1", checkpoint_path=None):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for chips, labels in train_loader:
            ...
            optimizer.zero_grad()
            loss = criterion(model(chips).squeeze(-1), labels_smooth)
        ...
        if current_score > best_score:
            ...
            if checkpoint_path:
                ...
                log(f"  Checkpoint saved (epoch {epoch}, {monitor.upper()}={best_score:.4f})")
        else:
            wait += 1
            if wait >= patience:
                log(f"Early stopping at epoch {epoch} (best val {monitor.upper()} = {best_score:.4f})")
                break
...
def plot_training_curves(history, split_name, plot_suffix=""):
    ...
    axes[0].set_title(f"Loss — {split_name}"); axes[0].legend()
    axes[1].plot(history["val_f1"],    color="green");  axes[1].set_title("Validation F1")
    axes[2].plot(history["val_auroc"], color="purple"); axes[2].set_title("Validation AUROC")
...
def show_failures(results, split_name, loader, plot_suffix="", n=4):
    ...
    if len(wrong) == 0:
        ...
        return
    top_wrong = wrong[np.argsort(-np.abs(probs[wrong] - 0.5))[:n]]
    dataset = loader.dataset
    fig, axes = plt.subplots(len(top_wrong), 2, figsize=(7, 3.5 * len(top_wrong)))
    if len(top_wrong) == 1: axes = axes[np.newaxis, :]
    fig.suptitle(f"Most Confident Errors — {split_name}", fontsize=13)
    for i, idx in enumerate(top_wrong):
        sid = dataset.sample_ids[idx]; chip = load_chip(sid)
        for j, band in enumerate(["VV", "VH"]):
            ax = axes[i, j]; b = chip[j]; valid_px = b[b != 0]
            vmin, vmax = np.percentile(valid_px, [2, 98]) if len(valid_px) > 0 else (b.min(), b.max())
            ax.imshow(b, cmap="gray", vmin=vmin, vmax=vmax)
            ax.set_title(f"{sid}\n{band} | true={labels[idx]} pred={probs[idx]:.2f}", fontsize=9)
            ax.axis("off")
...
if __name__ == "__main__":
    ...
    CONFIG = {
        ...
        "split": {
            ...
            "dropout":         0.4,
            "epochs":          80,
            "lr":              1e-3,       # higher than fine-tuning (5e-5) — needed from random init
            "weight_decay":    5e-4,
            "patience":        20,
    ...
    model_rand = build_resnet18_scratch(dropout=sc["dropout"]).to(device)
    model_rand, history_rand = train_model(
        model_rand, rand_train, rand_val, device,
        epochs=sc["epochs"], lr=sc["lr"], weight_decay=sc["weight_decay"],
        patience=sc["patience"], label_smoothing=sc["label_smoothing"],
        monitor=sc["monitor"],
        checkpoint_path=os.path.join(CKPT_DIR, "resnet18_scratch_random_best.pt"),
    )
    torch.save(model_rand.state_dict(),
               os.path.join(CKPT_DIR, "resnet18_scratch_random_final.pt"))
    plot_training_curves(history_rand, "Random Split (Scratch)", "_random")

    print("\n── Random split: evaluation ──")
    rand_results = full_eval(model_rand, rand_test, device,
                             "Random Split — Scratch (Test)", "_random")
    ...
    geo_vv_m, geo_vv_s, geo_vh_m, geo_vh_s = compute_channel_stats(geo_train_file, img_dir)
    ...
    model_geo = build_resnet18_scratch(dropout=sc["dropout"]).to(device)
    model_geo, history_geo = train_model(
        model_geo, geo_train, geo_val, device,
        epochs=sc["epochs"], lr=sc["lr"], weight_decay=sc["weight_decay"],
        patience=sc["patience"], label_smoothing=sc["label_smoothing"],
        monitor=sc["monitor"],
        checkpoint_path=os.path.join(CKPT_DIR, "resnet18_scratch_geo_best.pt"),
    )
    torch.save(model_geo.state_dict(),
               os.path.join(CKPT_DIR, "resnet18_scratch_geo_final.pt"))
    plot_training_curves(history_geo, "Geographic Split (Scratch)", "_geo")

    print("\n── Geographic split: evaluation ──")
    geo_results = full_eval(model_geo, geo_test, device,
                            "Geographic Split — Scratch (Mediterranean OOD)", "_geo")
    ...
    print("\n" + "=" * 55)
    print("  ResNet-18 (from scratch) — Results Summary")
    print("=" * 55)
